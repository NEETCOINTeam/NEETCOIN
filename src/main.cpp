...
const string strMessageMagic = "NEETCOIN Signed Message:\n";
...
int64_t GetProofOfStakeReward(const CBlockIndex *pindexLast, int64_t nCoinAge, int64_t nFees)
{
    ...
    // under four digits divided by 50000
    int uFourDigits = nBestHeight % 50000;

    if(nBestHeight >=10000 && nBestHeight <= 12100)
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 10;
    } 
    else if(nBestHeight >=20000 && nBestHeight <= 22100) 
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 10;
    } 
    else if(nBestHeight >=40000 && nBestHeight <= 42100) 
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 10;
    } 
    else if(nBestHeight >=70000 && nBestHeight <= 72100) 
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 10;
    } 
    else if(nBestHeight >=100000 && uFourDigits >= 0 && uFourDigits <= 2100) 
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 10;
    }
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    printf("check sum is %d", pindexNew->nStakeModifierChecksum);
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "NEETCOIN", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        //  Coinbase(hash=12630d16a9, nTime=1508863753, ver=1, vin.size=1, vout.size=1, nLockTime=0)
        //    CTxIn(COutPoint(0000000000, 4294967295), coinbase 00012a24323020466562203230313420426974636f696e2041544d7320636f6d6520746f20555341)
        //    CTxOut(empty)
        //  vMerkleTree: 12630d16a9
        ...
        assert(block.hashMerkleRoot == uint256("0x0a8875e41a01a01dd0f1c759d5da353ee92561c089d19ef939510d6eda44c096"));
