...
void TxToJSON(const CTransaction& tx, const uint256 hashBlock, Object& entry)
{
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++)
    {
        ...
        Object o;
        ScriptPubKeyToJSON(txout.scriptPubKey, o, true); // Insight API needs hex field
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2)
    {
        ...
        BOOST_FOREACH(Value& input, inputs)
        {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid NEETCOIN address: ")+input.get_str());
